...
[{
    ...
    if (_goggles in GVAR(respiratorMasks)) then {
        ...
        if (GVAR(lastSoundRan) + 3 < CBA_missionTime) then {
            ...
            // Optional disabling of mask sounds via: ace_player setVariable ["LXIR_Mission_enableMaskSounds", false];
            if (ace_player getVariable [QGVAR(enableMaskSounds), true] && {alive ace_player}) then {
                playSound "lxirr_gasmask_breath";
            };
        };
        // Add Mask
        if (GVAR(oldGlasses) != _goggles) then {
            playSound "lxirr_gasmask_on";
            "lxirr_gasmask_overlay" cutRsc ["lxirr_gasmask", "PLAIN", 1, false, false]; // Main effect
            if (_additionalEffect) then {
                "lxirr_gasmask_overlayAdditional" cutRsc ["RscCBRN_Regulator", "PLAIN", 1, false, false]; // Additional effect
            };
        };
    } else {
        // Mask Removal
        if (GVAR(oldGlasses) in GVAR(respiratorMasks)) then {
            playSound "lxirr_gasmask_off";
            "lxirr_gasmask_overlay" cutFadeOut 0;
            if (_additionalEffect) then {
                "lxirr_gasmask_overlayAdditional" cutFadeOut 0;
            };
        };
