/*
Function: lxir_commandcenter_fnc_initScreen
...
if (isNil "LXIR_CC_nextScreenID") then {
    LXIR_CC_nextScreenID = 0;
};

_object setVariable [format ["LXIR_CC_screen_%1_ID", _selection], LXIR_CC_nextScreenID, true];

LXIR_CC_nextScreenID = LXIR_CC_nextScreenID + 1;

_object setVariable [format ["LXIR_CC_screen_%1_on", _selection], false, true];
_object setVariable [format ["LXIR_CC_screen_%1_mode", _selection], "", true];
_object setVariable [format ["LXIR_CC_screen_%1_target", _selection], "", true];

// set default screen texture
_object setObjectTextureGlobal [_selection, "z\lxir\addons\media\images\cc_screen_standby.paa"];

[_object, _selection, _name, [_allowCam, _allowDrone, _allowSat, _allowMap]] remoteExec ["lxir_commandcenter_fnc_addScreenActions", 0, true];